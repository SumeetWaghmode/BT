...
*/
extern USER_EXT_DLL_API int BT7_CMHasImpactedItemPreCondition(METHOD_message_t* msg, va_list args) {
	int		iStatus			= ITK_ok;
	...
	if(bIsPrimaryDerivedFromChangeNoticeRevision && bIsSecondaryDerivedFromItemRevision) {
		...
		// Status OK?
		if(iStatusCount > 0 && (strcmp(sLatestStatusName, STATUS_TYPE_10) == 0 || strcmp(sLatestStatusName, STATUS_TYPE_60) == 0 || strcmp(sLatestStatusName, STATUS_TYPE_PRERELEASEPLUS) == 0) ||strcmp(sLatestStatusName, STATUS_TYPE_90) == 0) {
			// OK
		} else {
			// Error: Must have status 10 or 60 or 90
			iStatus = 919721;
		...
		if(iStatus == ITK_ok) {
			...
			if(lIsLatestRevOfThisStatus == false) {
				if(strcmp(sLatestStatusName, STATUS_TYPE_10) == 0) {
					iStatus = 919726;
				} else if(strcmp(sLatestStatusName, STATUS_TYPE_60) == 0) {
					iStatus = 919727;
				} else if(strcmp(sLatestStatusName, STATUS_TYPE_90) == 0) {
					iStatus = 919734;
				}else if(strcmp(sLatestStatusName, STATUS_TYPE_PRERELEASEPLUS) == 0) {
					iStatus = 919734;
				}
				else {
					iStatus = 919721;
