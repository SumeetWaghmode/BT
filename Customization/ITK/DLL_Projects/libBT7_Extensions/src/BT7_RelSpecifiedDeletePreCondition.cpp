...
/**
    ...
    @param tag_t object
*/
extern USER_EXT_DLL_API int BT7_RelSpecifiedDeletePreCondition(METHOD_message_t* msg, va_list args) {
	int		iStatus			= ITK_ok;
	tag_t	tRelationTag	= va_arg(args, tag_t);

	if(DEBUG == 1) printf(" > \n");
	if(DEBUG == 1) printf(" > BT7_RelSpecifiedDeletePreCondition \n");
	if(DEBUG == 1) printf(" > \n");
		...
		if(
			(
				(isStatusEmptyOr5(tPrimaryLatestStatus) || (isStatus10(tPrimaryLatestStatus) && isCommonComponent(tPrimaryTag)))
				 &&
				(isStatusEmptyOr5(tSecondaryLatestStatus) || isStatus10(tSecondaryLatestStatus) || isStatus60(tSecondaryLatestStatus) || isStatus90(tSecondaryLatestStatus) || isStatus98(tSecondaryLatestStatus))
			)
			 ||
			( !isSoftware(tPrimaryTag) && isStatus10(tPrimaryLatestStatus) && (isStatusEmptyOr5(tSecondaryLatestStatus)) )
			 ||
			( !isSoftware(tPrimaryTag) && isStatus60(tPrimaryLatestStatus) && (isStatusEmptyOr5(tSecondaryLatestStatus)) )
			||
