...
logical attachBomChildsForAnulling(tag_t tSelectedItemRevision, vector<tag_t> *vecTcObjectsToBeAnulled, vector<tag_t> *vecTcObjectsAlreadyTested, logical lIsTopAssembly)
{
	...
	for ( int iIdx1 = 0; iIdx1 < iItemRevCount; iIdx1++ )
	{
		...
		if ( ( lIsTopAssembly == true ) || object_is_of_class_or_desc(taItemRevList[iIdx1], TC_ITEMREVISION, &tClassTag) )/* Fix for SBTPRPDMBT-152
													INT: Test Case failed "Set Status Annulled on Component Structure used in Annulled Objects"*/
		{
...
int SBT_AH_generate_cancel_form_and_attach_to_workflow(EPM_action_message_t msg)
{
	...
	/*Fix for issue: 0SBT-PR-01PDM4BT/ SBTPRPDMBT-134 The instance is not locked: Handler "SBT-AH-attach-item-and-all-revs"::
	Added AOM_refresh & AOM_save_without_extensions as AOM_save is deprecated and will be removed from Tc13*/
...
int SBT_AH_cleanup_relations(EPM_action_message_t msg)
{
	...
	// Commented for issue SBTPRPDMBT-232
...
int SBT_AH_set_latest_release_status(EPM_action_message_t msg)
{
	...
	for ( int iIdx1 = 0; iIdx1 < iTargetCount; iIdx1++ )
	{
		if(object_is_of_class_or_desc(pTargetTags[iIdx1], sParamObjectType, &tRevClassTag))
		{
			...
			// Get all status
			ITK(WSOM_ask_release_status_list(pTargetTags[iIdx1], &iStatusCount, &taStatusList) );
			/*Fix for issue 0SBT-PR-01PDM4BT/ SBTPRPDMBT-165 INT: WF "BT7_Reactivate" TC Crash
			*Added check for iStatusCount as for ItemRevision(i.e. with only Annulled status) 
			*iStatusCount = 0 leading to TC crash while evaluating AOM_UIF_ask_value*/
			if(iStatusCount>0 && taStatusList != NULL)
			{
...
EPM_decision_t SBT_RH_part_rev_is_solution_item_of_ECO(EPM_action_message_t msg)
{
	...
	for(int inx=0;inx<targetCount;inx++)	
	{
		...
		if ( ( strcmp ( sGroupname, "110_MDesign_C" ) == 0 ) || ( strcmp ( sGroupname, "110_MDesign_D" ) == 0 ) || ( strcmp ( sGroupname, "Common Component Engineering" ) == 0 ) || ( strcmp ( sGroupname, "100_Library_A" ) == 0 ) || ( strcmp ( sObjectType, "BT7_ToolRevision" ) == 0 ) || ( strcmp ( sObjectType, "BT7_ToolPartRevision" ) == 0 )
			|| ( ( strcmp ( sGroupname, "110_TestTech_A" ) == 0 ) && ( strcmp ( sObjectType, "BT7_DocumentRevision" ) == 0 ) ))	
		{
		...
		{
			/* Fix for issue: 0SBT-DO-01PDM4BT|SBTDOP-86 "Serial Release" ECOSBTDOP-116:: 
			Added if block to check if is target tag is child of BT7_DocumentRevision & Part Revision*/
			if(object_is_of_class_or_desc(attachmentTag[inx], TC_PARTREVISION, &tclass) ||
			object_is_of_class_or_desc(attachmentTag[inx], SBT_BT_DOCUMENTREVISION, &tclass))
			{
				...
				MEM_free(ecoValue);
				MEM_free(ecoRevTag);
			}//end of if block to check if is target tag is child of BT7_DocumentRevision & Part Revision
...
int SBT_AH_change_object_type( EPM_action_message_t msg ) {
	...
	else if( object_is_of_class_or_desc( pTargetTags[0], "ItemRevision", &objectClassIdTag ) ) {
		int * attachTypes = ( int * ) MEM_alloc( 1 * sizeof( int ) );
		tag_t * attachment = ( tag_t * ) MEM_alloc( 1 * sizeof( tag_t ) );
		
		/* Switch Attachment from Revision to Item */
		ITK( ITEM_ask_item_of_rev( pTargetTags[ 0 ], &sourceItemTag ) );
		ITK( AOM_refresh( rootTaskTag, true ) );
		ITK( EPM_remove_attachments( rootTaskTag, iTargetCount, pTargetTags ) );
		ITK( EPM_add_attachments( rootTaskTag, 1, attachment, attachTypes ) );
		ITK( AOM_save_without_extensions( rootTaskTag ) );
		ITK( AOM_refresh( rootTaskTag, false ) );
		
		SAFE_SM_FREE( attachTypes );
		SAFE_SM_FREE( attachment );
	}
	else {
	...
	for( int iIdx = 0; iIdx < sourceRevCount; iIdx++ ) {
		...
		for( int iIdx1 = 0; iIdx1 < targetRevCount; iIdx1++ ) {
			...
			if( strcmp( sourceRevID, targetRevID ) == 0 ) {
				...
				tag_t*	targetRevInSingleList = ( tag_t * ) MEM_alloc( 1 * sizeof( tag_t ) );
				
				targetRevInSingleList[ 0 ] = targetRevList[ iIdx1 ];
				ITK( AOM_ask_value_tags( sourceRevList[ iIdx ], "project_list", &projectCount, &projectTags ) );
				ITK( PROJ_assign_objects( projectCount, projectTags, 1, targetRevInSingleList ) );

				SAFE_SM_FREE( projectTags );
				SAFE_SM_FREE( targetRevInSingleList );
...
/**
 * @brief
 * Added Rule handler for Fix of Issue #SBTPRPDMBT-203
 * This Rule Handler verifies if the target revisions have same released status.
 */
EPM_decision_t SBT_RH_check_targets_same(EPM_rule_message_t msg)
{
	int				retcode			= ITK_ok;
	int				iTargetCount	= 0;
	char*			currentWF		= NULL;//OF
	const char		*theFunction	= "SBT_RH_check_targets_same";
	EPM_decision_t	sDecision		= EPM_nogo;
	tag_t			rootTaskTag		= NULLTAG;
	tag_t*			pTargetTags		= NULLTAG;
	logical			lStatusOK		= true;
	...
	vector<std::string> vecTargetRevStatus;

	/* Get the target objects*/
	ITK(EPM_ask_root_task(msg.task, &rootTaskTag) );// get the root task

	// Get the workflow name 
	ITK ( AOM_ask_name (rootTaskTag,&currentWF));
	...
	if(iTargetCount>0){
		...
		for ( int iIdx1 = 0; iIdx1 < iTargetCount; iIdx1++ ){
			...
			//Checks if target is child of ItemRevision
			if(object_is_of_class_or_desc(pTargetTags[iIdx1],TC_ITEMREVISION,&classID)){
				
				// Get the status
				int		iStatusCount		=	0;
				char*	sStatusName		    =	NULL;//OF
				string	curStatus			=	"";				

				// Get latest status
				askLatestReleaseStatus(pTargetTags[iIdx1], &sStatusName, &iStatusCount);

				if(iStatusCount==0){// check if status count is 0
					if(isRevIsWIP(pTargetTags[iIdx1],currentWF)){// checks if current iterated target revision in WIP
						vecTargetRevStatus.push_back(REV_IN_WIP);
					}
					else{
						vecTargetRevStatus.push_back(curStatus);// if there is without status and not in WIP
					}
				}
				else{
					curStatus= std::string(sStatusName);

					vecTargetRevStatus.push_back(curStatus);
				}
				if(sStatusName != NULL){
				SAFE_SM_FREE(sStatusName);
				}
			}
		}// end of Loop over all target attachments
	}// end of Count check

	//checks if vector contains all same elements
	if ( adjacent_find( vecTargetRevStatus.begin(), vecTargetRevStatus.end(), not_equal_to<string>() ) == vecTargetRevStatus.end() )
	{
		cout << "\nAll elements are equal each other" << std::endl;
		sDecision = EPM_go;
	}
	else
	{
		printf("\n > ERROR: In Rule Handler 'SBT_RH_check_targets_same' :The target revisions doesn't have same latest status ! \n");
		ITK(EMH_store_error(EMH_severity_error, 919735) );
	}

	/* Free */
	SAFE_SM_FREE(currentWF);
	SAFE_SM_FREE(pTargetTags);
...
/**
 * @brief
 * Verifies if the target revision is in Work in Progress(not considers currently applied workflow).
 */
logical isRevIsWIP(tag_t inputItemRev, char* curWFname){
	int		wfCount			= 0	;
	int		counter			= 0	;
	logical	isWIP			= false;
	char*	WFname			= NULL;//OF
	tag_t*	propValuesTags	= NULL;//OF
	//Get number and tag of values in property fnd0StartedWorkflowTasks
	ITK ( AOM_ask_value_tags(inputItemRev,"fnd0StartedWorkflowTasks",&wfCount,&propValuesTags));
					
	//Loop all over number of values to found same workflow name
	for(int inx1=0;inx1<wfCount;inx1++)
	{								
		ITK ( AOM_ask_name (propValuesTags[inx1],&WFname));	
		//compare workflow name with that values in property fnd0StartedWorkflowTasks
		if (!(strcmp(curWFname,WFname)==0))
		{
			counter++;// increment counter by 1 if workflow name is same
		}
