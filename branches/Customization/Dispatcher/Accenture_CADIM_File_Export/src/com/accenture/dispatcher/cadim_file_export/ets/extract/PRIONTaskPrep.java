...
================================================================================*/
//==== Package  ================================================================
package com.accenture.dispatcher.cadim_file_export.ets.extract;
...
//==== Class ====================================================================
/**
 * Custom TaskPrep class for proetojt translator. This sub class prepares a
 * proetojt translation task. This is a configuration specified class based on
 * provider name and translator name in DispatcherClient property file which
 * creates the proe specific translation request by preparing the data for
...
public class PRIONTaskPrep extends DefaultTaskPrep {
	...
	private static Registry registry = Registry
			.getRegistry("com.accenture.dispatcher.cadim_file_export.tstk.translator.priontranslator");
	private String m_objectType = "";
	private Dataset m_FinalDataset = null;

	private String FILE_UID = "";
	private String CADIM_ENTRY_UID = "";
	...
	public TranslationTask prepareTask() throws Exception {
		...
		// Change fro upgrade
		String translatorArgsKeys[] = request.getPropertyObject(
				TranslationRequest.TRANSLATION_ARGS_KEYS.toString()).getStringArrayValue();
		String translatorArgsData[] = request.getPropertyObject(
				TranslationRequest.TRANSLATION_ARGS_DATA.toString()).getStringArrayValue();
		for (int i = 0; i < translatorArgsKeys.length; i++) {
			if (translatorArgsKeys[i].equalsIgnoreCase("FileUID")) {
				FILE_UID = translatorArgsData[i];
			} else if (translatorArgsKeys[i].equalsIgnoreCase("CadimEntryUID")) {
				CADIM_ENTRY_UID = translatorArgsData[i];
			} else {
				//
			}
		}
		...
		PRIONConfigManager.RetrieveTransaltorConfig(
				request.getPropertyObject(TranslationRequest.TRANSLATOR_NAME)
						.getDisplayableValue(), PRIONDSPDatasetBean_vector,
				true, m_zTaskLogger);
		...
		if (!((PRIONDSPDatasetBean) PRIONDSPDatasetBean_vector.get(0))
				.getDataSetType().toLowerCase().equals("none")) {
			...
			ModelObject secondary_objs[] = request.getPropertyObject(
					TranslationRequest.SECONDARY_OBJS)
			...
			for (int i = 0; i < primary_objs.length; i++) {
				...
				// Changed for upgrade
				m_zTaskLogger.debug("PRIONTaskPrep -> PRIMARY OBJECT TYPE IS: "
						+ transPrimaryObject_dataset.get_object_type());
				// Changed for upgrade
				m_zTaskLogger
						.debug("PRIONTaskPrep -> SECONDARY OBJECT TYPE IS: "
								+ transSecondaryObject_itemrev.get_object_type());
				...
				if (transPrimaryObject_dataset.get_object_type().toLowerCase().equals("ugmaster")) {
					// Changed for upgrade
					if (request.getPropertyObject(TranslationRequest.PRIMARY_OBJS)
							.getDisplayableValue().contains(";")) { // It
						// means,
						// the
						// primary
						// object
						...
						// version
						m_zTaskLogger
								.debug("PRIONTaskPrep -> The Primary object is a UG VERSION!!!");
						ModelObject mos[] = transSecondaryObject_itemrev
								.get_IMAN_specification();
						for (int xx = 0; xx < mos.length; xx++) {
							ModelObject mo = mos[xx];
							// Changed for Upgrade
							m_zTaskLogger
									.debug("PRIONTaskPrep ->     Object under ItemRev: "
											+ mo.getTypeObject().getName());
							if (mo.getTypeObject().getName().toLowerCase()
									.equals("ugmaster")) {
								// Changed for Upgrade 
								m_zTaskLogger
										.debug("PRIONTaskPrep ->        current_name: "
												+ mo.getPropertyObject("current_name")
														.getDisplayableValue());
								m_zTaskLogger
										.debug("PRIONTaskPrep ->        PO current_name: "
												+ transPrimaryObject_dataset
														.getPropertyObject(
																"current_name")
														.getDisplayableValue());
								if (mo.getPropertyObject("current_name")
										.getDisplayableValue()
										.equals(transPrimaryObject_dataset
												.getPropertyObject("current_name")
												.getDisplayableValue())) {
				...
				for (int x = 0; x < PRIONDSPDatasetBean_vector.size(); x++) {
					...
					// Changed for upgrade
					if (((PRIONDSPDatasetBean) PRIONDSPDatasetBean_vector
							.get(x)).getDataSetType().equals(
							transPrimaryObject_dataset.get_object_type())) {
				...
				m_zTaskLogger.debug("PRIONTaskPrep -> PRIMARY_OBJS: "
						+ request.getPropertyObject(TranslationRequest.PRIMARY_OBJS)
								.getDisplayableValue());
				...
				if (datasetBean.getSourceFileLocation().equals("SAME")) {
					...
					String finalDatasetRelation = datasetBean
							.getSourceFileRelationWithSon();

					getSon(finalDatasetRelation, primary_objs[i], m_objectType);

				} else {
					m_zTaskLogger
							.debug("PRIONTaskPrep -> I do not know if the sourcefile is in the same dataset or in the son!!");
					m_zTaskLogger.debug("              -> Value: "
							+ datasetBean.getSourceFileLocation());
					continue;
				}
				...
				for (int j = 0; j < contexts.length; j++) {
					...
					if (contexts[j] instanceof ImanFile) {
						...
						if ((TranslationRequest.getFileToStaging(zIFile,
								stagingLoc))
								.getName()
								.toLowerCase()
								.endsWith(
										(registry
												.getString("PRIONTaskPrep.source.file.extension."
														+ m_objectType))
												.toLowerCase())) {
							break;
						}
				...
				if (zIFile == null) {
					throw new Exception(
							"PRIONTaskPrep ->  -> No named reference found for "
									+ m_FinalDataset.get_object_string());
				}

				// Export the file to the staging directory
				File zFile = TranslationRequest.getFileToStaging(zIFile,
						stagingLoc);

				// Once we have the sourceFile on the Staging dir, we will write
				// the PRION TASK configuration file
				PRIONConfigManager.writePRIONTaskConfiguration(false, request,
						transPrimaryObject_dataset,
						transSecondaryObject_itemrev, stagingLoc, FILE_UID,
						CADIM_ENTRY_UID, m_FinalDataset.get_current_name(),
						zFile.getName(), datasetBean);
				...
				if (datasetBean.getFinalFileName().endsWith(m_scSourceFileExt)) {
					this.m_zTaskLogger.debug("        -> zFileName: "
							+ zFile.getName());
					this.m_zTaskLogger.debug("        -> finalName: "
							+ datasetBean.getFinalFileName());
					if (!zFile
							.getName()
							.toLowerCase()
							.equals(datasetBean.getFinalFileName()
									.toLowerCase())) {
						renameFile(
								stagingLoc + File.separator + zFile.getName(),
								stagingLoc + File.separator
										+ datasetBean.getFinalFileName());
						this.m_zTaskLogger
								.debug("        -> THE FILE HAS BEEN RENAMED TO: "
										+ datasetBean.getFinalFileName());
					}
				} else {
					this.m_zTaskLogger.debug("        -> zFileName: "
				...
				String sanitizedFilename = getSanitizedFilename(
						registry.getString("PRIONTaskPrep.sanitize.BadChars"),
						fileNameExtTrim);
				...
				if (sanitizedFilename != null) {
					...
					exportFilename = sanitizedFilename
							+ registry
									.getString("PRIONTaskPrep.sanitize.replacementChar")
							+ i + m_scSourceFileExt;

					// If the datasetBean has already included the file
					// extension in the filename
					// we do not have to include it agagin.
					if (datasetBean.getFinalFileName().endsWith(
							m_scSourceFileExt)) {
						...
						renameFile(
								stagingLoc + File.separator
										+ datasetBean.getFinalFileName(),
								stagingLoc + File.separator + exportFilename);
					} else {
						renameFile(
								stagingLoc + File.separator
										+ datasetBean.getFinalFileName()
										+ m_scSourceFileExt, stagingLoc
										+ File.separator + exportFilename);
					}
					this.m_zTaskLogger
							.debug("        -> SANITIZED: THE FILE HAS BEEN RENAMED TO: "
									+ exportFilename);

					// And overwrite the PRION TASK configuration file
					PRIONConfigManager.writePRIONTaskConfiguration(true,
							request, transPrimaryObject_dataset,
							transSecondaryObject_itemrev, stagingLoc, FILE_UID,
							CADIM_ENTRY_UID, m_FinalDataset.get_current_name(),
							exportFilename, datasetBean);
				} else {
				...
				zTransTask = prepTransTask(zTransTask, m_FinalDataset,
						transSecondaryObject_itemrev, exportFilename, true,
						true, datasetBean.getTranslatedFileExtension(), 0, null);
			...
			// ModelObject primary_objs[] = request.getProperty(
			// TranslationRequest.PRIMARY_OBJS).getModelObjectArrayValue();
			// ItemRevision transSecondaryObject_itemrev = (ItemRevision)
			// primary_objs[0];

			m_zTaskLogger.debug("PRIONTaskPrep -> DATASET: - NONE -");
			// m_zTaskLogger.debug("ITEM ID: "
			// + transSecondaryObject_itemrev.get_current_id());
			// m_zTaskLogger.debug("ITEM REVISION: "
			// + transSecondaryObject_itemrev.get_current_revision_id());

			// If we don't want to process any dataset we will write the
			// PRIONTraskConfiguration File and finish.
			PRIONConfigManager.writePRIONTaskConfigurationNONE(request,
					stagingLoc, FILE_UID, CADIM_ENTRY_UID,
	...
	protected String getSanitizedFilename(String scBadString, String scFileName) {
		this.m_zTaskLogger.debug("PRIONTaskPrep ->  -> Sanitizing: "
				+ scFileName);
		...
		for (int j = 0; j < scBadChar.length; j++) {
			this.m_zTaskLogger
					.debug("PRIONTaskPrep ->  -> Searching Bad Character: "
							+ scBadChar[j]);
			if (scFileName.indexOf(scBadChar[j]) == -1) {
				continue;
			}
			this.m_zTaskLogger
					.debug("PRIONTaskPrep ->  ->       - Replacing by: "
							+ registry
									.getString("PRIONTaskPrep.sanitize.replacementChar"));
		...
		this.m_zTaskLogger.debug("PRIONTaskPrep ->  -> Sanitized FileName: "
				+ scFileName);
		return scFileName;
	}

	@SuppressWarnings({ "rawtypes", "unchecked" })
	protected void getSon(String finalDatasetRelation,
			Object transPrimaryObject_dataset, String transPrimaryObject_type) {
		try {
			Class auxClass = null;
			try {
				auxClass = Class
						.forName("com.teamcenter.soa.client.model.strong."
								+ transPrimaryObject_type);
			} catch (ClassNotFoundException ex) {
				try {
					this.m_zTaskLogger
							.debug("PRIONTaskPrep ->  com.teamcenter.soa.client.model.strong."
									+ transPrimaryObject_type + " NOT FOUND");
					if (transPrimaryObject_type.equals("IdeasDrawing")) {
						// auxClass =
						// Class.forName("com.prion.ets.util.Dataset");
						auxClass = Class
								.forName("com.teamcenter.soa.client.model.strong.Dataset");
					} else {
						auxClass = Class
								.forName("com.teamcenter.soa.client.model.strong.Dataset");
					}
				} catch (ClassNotFoundException ex1) {
					Logger.getLogger(PRIONTaskPrep.class.getName()).log(
							Level.SEVERE, null, ex1);
				}
			}

			this.m_zTaskLogger.debug("PRIONTaskPrep ->  Class is "
					+ auxClass.getName());

			Object auxObject = transPrimaryObject_dataset;
			transPrimaryObject_dataset.getClass().cast(auxObject);

			Method metodo = auxClass.getMethod("get_" + finalDatasetRelation,
					new Class[] {});
			ModelObject[] auxModelObject = (ModelObject[]) metodo.invoke(
					auxObject, null);

			m_zTaskLogger.debug("PRIONTaskPrep ->  -> " + auxModelObject.length
					+ " Objects found using get_" + finalDatasetRelation
					+ " on " + transPrimaryObject_type);
			...
			Logger.getLogger(PRIONTaskPrep.class.getName()).log(Level.SEVERE,
					null, ex);
	...
	public TranslationTask prepareTaskNONE() throws Exception {
		...
		File zDummyFile = new File(this.stagingLoc + File.separatorChar
				+ "dummy.txt");
		zDummyFile.createNewFile();
		TranslationTask zTransTask = addRefIdToTask(
				prepTransTask(null, null, null, "dummy.txt", false, false,
						null, 0, null), 0);
		// Changed for TC upgrade
		return addOptions(zTransTask, "TASK_ID",
				this.request.getPropertyObject("taskID").getStringValue());
	}
